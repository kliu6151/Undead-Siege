...
export default class HealerBehavior extends NPCBehavior {
  ...
  public initializeAI(owner: NPCActor, opts: Record<string, any>): void {
    ...
    this.addStatus(
      ...
      new HasItem(owner, new BasicFinder<Item>(null, ItemFilter(Healthpack)))
    );

    // Check if a lowhealth ally exists in the scene
    let lowhealthAlly = new BasicFinder<Battler>(
      null,
      BattlerActiveFilter(),
      BattlerGroupFilter([owner.battleGroup])
    );
    this.addStatus(
      HealerStatuses.ALLY_EXISTS,
      new TargetExists(scene.getBattlers(), lowhealthAlly)
    );
    ...
    healAlly.targetFinder = new BasicFinder<Battler>(
      ClosestPositioned(owner),
      BattlerActiveFilter(),
      BattlerGroupFilter([owner.battleGroup]),
      BattlerHealthFilter(0, 5)
    );
