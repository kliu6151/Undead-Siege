...
export default class GuardBehavior extends NPCBehavior {
  ...
  protected initializeStatuses(): void {
    ...
    this.addStatus(
      GuardStatuses.LASERGUN_EXISTS,
      new TargetExists(
        scene.getLaserGuns(),
        new BasicFinder<Item>(null, ItemFilter(LaserGun), VisibleItemFilter())
      )
    );
    // Add a status to check if the guard has a lasergun
    this.addStatus(
      GuardStatuses.HAS_WEAPON,
      new HasItem(this.owner, new BasicFinder(null, ItemFilter(LaserGun)))
    );
  ...
  protected initializeActions(): void {
    ...
    pickupLaserGun.targetFinder = new BasicFinder<Item>(
      ClosestPositioned(this.owner),
      VisibleItemFilter(),
      ItemFilter(LaserGun)
    );
