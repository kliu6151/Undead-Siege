...
export default class ShootLaserGun extends NPCAction {
  ...
  public performAction(target: TargetableEntity): void {
    ...
    if (
      this.timer.isStopped() &&
      this.lasergun !== null &&
      this.lasergun.inventory !== null &&
      this.lasergun.inventory.id === this.actor.inventory.id
    ) {
      ...
      this.lasergun.laserEnd.copy(
        this.getLaserEnd(
          this.actor.getScene().getWalls(),
          this.lasergun.laserStart,
          this.lasergun.direction
        )
      );
  ...
  public onEnter(options: Record<string, any>): void {
    ...
    let lasergun = this.actor.inventory.find(
      (item) => item.constructor === LaserGun
    );
    ...
    for (let col = minIndex.x; col <= maxIndex.x; col++) {
      for (let row = minIndex.y; row <= maxIndex.y; row++) {
        if (walls.isTileCollidable(col, row)) {
          ...
          if (
            hit !== null &&
            start.distanceSqTo(hit.pos) < start.distanceSqTo(end)
          ) {
            end = hit.pos;
          }
        }
